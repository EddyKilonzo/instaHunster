...
logo = fig.renderText(f'fucked by joker')
print(logo)

nnn = random.choice([R,X,F,B,K,V])
good_hot,bad_hot,good_ig,bad_ig,check,mj,ids=0,0,0,0,0,0,[]
tok = input('•       EnTeR BoT ToKeN ->'.format(B,C,V,K))
print("DECODE BY JOKER | @hackingareas • \r")
iD = input('•EnTeR UseR iD : {}'.format(B,C,V,K))
os.system('clear')
def cookie(email):
...
def check_email(email):
	...
	bo = f'\x1b[38;5;{b}m'
	check+=1
	sys.stdout.write(f"\r  {C}✓ HiTz -> {F}{good_hot} || {C}✓ FucKeD -> {R}{bad_ig}  {C}|| TooL MaDe By -> @hackingareas\r")
	sys.stdout.flush()
